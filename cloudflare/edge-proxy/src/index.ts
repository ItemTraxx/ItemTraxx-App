...
const sanitizeRequestHeaders = (request: Request, anonKey: string) => {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!originAllowed) {
      return buildError(403, "Origin not allowed", headers);
    }

    const functionName = getFunctionName(url.pathname);
    if (!functionName) {
      return buildError(404, "Not found", headers);
    }

    const allowedFunctions = parseCsv(env.ALLOWED_FUNCTIONS);
    if (allowedFunctions.length > 0 && !allowedFunctions.includes(functionName)) {
      return buildError(403, "Function not allowed", headers);
    }

    if (!env.SUPABASE_URL || !env.SUPABASE_ANON_KEY) {
      return buildError(500, "Proxy misconfiguration", headers);
    }
    ...
    const proxiedHeaders = sanitizeRequestHeaders(request, env.SUPABASE_ANON_KEY);
